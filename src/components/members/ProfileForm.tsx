...
export default function ProfileForm({ profile, locale }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!payload.medical_conditions) payload.medical_conditions = null;
    ...
    <form onSubmit={handleSave} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              Member since {profile.joined_date
                ? new Date(profile.joined_date).toLocaleDateString('en-IN', { day:'numeric', month:'long', year:'numeric' })
                : 'recently'}
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input name="date_of_birth" type="date" value={form.date_of_birth} onChange={handleChange} className="input" />
          </div>
          <div>
        ...
        <div>
          ...
          <input name="emergency_contact" value={form.emergency_contact} onChange={handleChange} className="input" placeholder="Name and phone number" />
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Shared only with your instructor. Helps us adapt your practice safely.</p>
        </div>
        <div>
          <label className="label">Medical Conditions / Injuries</label>
          <textarea
            ...
            placeholder="e.g. Lower back pain, high blood pressure, recent knee surgery… Leave blank if none."
          />
        </div>
      </div>
